...
class ReportingAccessGateway extends QueryableGateway
{
    ...
    public function queryActiveCriteriaGroupsByPerson(QueryCriteria $criteria, $gibbonReportingScopeID, $gibbonPersonID, $allStudents = false)
    {
        ...
        $query = $this
            ...
            "(SELECT COUNT(*) FROM gibbonReportingProgress as progress JOIN gibbonPerson AS student ON (student.gibbonPersonID=progress.gibbonPersonIDStudent) JOIN gibbonCourseClassPerson AS students ON (students.gibbonCourseClassID=progress.gibbonCourseClassID AND student.gibbonPersonID=students.gibbonPersonID) WHERE progress.gibbonCourseClassID=gibbonCourseClassPerson.gibbonCourseClassID AND progress.gibbonReportingScopeID=:gibbonReportingScopeID AND progress.status='Complete' AND students.role='Student' $onlyFullStudents) as progressCount",
            "(SELECT COUNT(*) FROM gibbonReportingProgress as progress JOIN gibbonPerson AS student ON (student.gibbonPersonID=progress.gibbonPersonIDStudent) JOIN gibbonCourseClassPerson AS students ON (students.gibbonCourseClassID=progress.gibbonCourseClassID AND student.gibbonPersonID=students.gibbonPersonID) WHERE progress.gibbonCourseClassID=gibbonCourseClassPerson.gibbonCourseClassID AND progress.gibbonReportingScopeID=:gibbonReportingScopeID AND progress.status='Complete' AND (student.status='Left' OR student.dateStart>:today OR student.dateEnd<:today OR students.role='Student - Left')) as leftCount"])
